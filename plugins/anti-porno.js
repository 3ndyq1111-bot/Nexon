...
handler.before = async function (m, { conn, isAdmin, isBotAdmin, isOwner }) {
    ...
    if (!isBotAdmin) {
        console.log("Bot non admin → antiporno non può cancellare/kickare")
        return true
    }
    ...
    try {
        ...
        let raw, partial, sexual, erotica, very_suggestive, suggestive
        if (type === 'video') {
            ...
            very_suggestive = Math.max(...frames.map(f => f.nudity?.very_suggestive || 0))
            suggestive = Math.max(...frames.map(f => f.nudity?.suggestive || 0))
        } else {
        ...
        if (isHighRisk) {
            ...
            if (user.warn < 3) {
                await conn.sendMessage(m.chat, {
                    text: `*@${senderTag}* 🚫 Contenuto NSFW/ rilevato!\n\nAvvertimento *${user.warn}/3*`,
                    mentions: [m.sender]
                })
            } else {
                user.warn = 0
